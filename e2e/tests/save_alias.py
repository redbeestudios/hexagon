...
test_folder_path = os.path.realpath(os.path.join(
    __file__, os.path.pardir, os.path.pardir, 'save_alias'
))
...
def test_save_alias():
    ...
    assert_process_output(process, [
        ['Hi, which tool would you like to use today?', '⬡ Save Last Command as Linux']
    ])
