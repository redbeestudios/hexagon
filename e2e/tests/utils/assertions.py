...
last_output_file_path = os.path.realpath(os.path.join(__file__, os.path.pardir, os.path.pardir, os.path.pardir, 'last-output.txt'))
...
def _save_last_output_and_raise(process: subprocess.Popen, lines_read: List[str], assertion_error: AssertionError):
  process.kill()
  lines = [*lines_read, *process.stdout.readlines()]
  _save_last_output(lines)
  raise assertion_error

def assert_process_output(process: subprocess.Popen, expected_output: List[str]):
  line_index = 0
  lines_read = []
  # Read from stdout until assertion fails or all expected lines are read
  # Attempting to read more lines than available will result in a deadlock, we should put a short timeout here. Test-level tiemouts will either be too short or too long because a test can have many output assertions, especially if a test is a long I/O flow
